...
/**
 * Get movies by genre (or all movies when genreId is null)
 * @param {number|null} genreId - The genre ID, or null for all genres
 * @param {number} page - Page number for pagination
 * @param {object} options - Additional discover options (e.g. sort_by)
...
/**
 * Get TV shows by genre (or all TV shows when genreId is null)
 * @param {number|null} genreId - The genre ID, or null for all genres
 * @param {number} page - Page number for pagination
 * @param {object} options - Additional discover options (e.g. sort_by)
 * @returns {Promise<object>} - TV shows of specified genre
 */
export async function getTVShowsByGenre(genreId, page = 1, options = {}) {
  const params = {
    page,
    sort_by: 'popularity.desc',
    ...options,
...
/**
 * Get streaming watch providers for a media type
 * @param {string} mediaType - 'movie' or 'tv'
 * @param {string} region - ISO 3166-1 country code (e.g. 'US')
 * @returns {Promise<object>} - List of watch providers
 */
export async function getWatchProviders(mediaType = 'movie', region = 'US') {
  return fetchFromTMDb(`/watch/providers/${mediaType}`, { watch_region: region });
}

/**
 * Get the watch providers available for a specific title in a region
 * @param {string} mediaType - 'movie' or 'tv'
 * @param {number|string} id - TMDb ID
 * @param {string} region - ISO 3166-1 country code (e.g. 'US')
 * @returns {Promise<object>} - Watch provider availability keyed by region
 */
export async function getWatchProvidersForContent(mediaType, id, region = 'US') {
  return fetchFromTMDb(`/${mediaType}/${id}/watch/providers`, { watch_region: region });
}

/**
 * Get titles similar to a given movie or show
 * @param {string} mediaType - 'movie' or 'tv'
 * @param {number|string} id - TMDb ID
 * @param {number} page - Page number
 * @returns {Promise<object>} - Similar titles
 */
export async function getSimilar(mediaType, id, page = 1) {
  return fetchFromTMDb(`/${mediaType}/${id}/similar`, { page });
}
