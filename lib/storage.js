...
export function addToMyList(item) {
  ...
  try {
    ...
    // Add timestamp for sorting (most recently added first) and a default
    // status so items can be organized into My List / Planning / Watched.
    const itemWithTimestamp = {
      ...item,
      media_type: mediaType,
      status: item.status || 'list',
      addedAt: Date.now()
    };
...
/**
 * Change the status of an item in the user's watchlist
 * ('list', 'planning', or 'watched')
 * @param {number} itemId - The TMDb ID
 * @param {string} mediaType - 'movie' or 'tv'
 * @param {string} status - New status for the item
 * @returns {Array} - Updated list
 */
export function setItemStatus(itemId, mediaType, status) {
  if (typeof window === 'undefined') {
    return [];
  }
  ...
  try {
    ...
    const updatedList = currentList.map(item =>
      item.id === Number(itemId) && item.media_type === mediaType
        ? { ...item, status }
        : item
    );
    localStorage.setItem(MY_LIST_KEY, JSON.stringify(updatedList));
...
/**
 * Clamp a watch-progress fraction (0..1) so stored values can never overflow.
 * @param {number} value - Raw progress value
 * @returns {number} - Clamped 0..1 value
 */
...
export function saveWatchProgress(item) {
  ...
  const entry = {
    ...item,
    media_type: mediaType,
    // "Just started" — the watch page bumps this over time; cards render a
    // progress bar from it.
    progress: typeof item.progress === 'number' ? clampProgress(item.progress) : 0.03,
    watchedAt: Date.now(),
  };
...
export function updateWatchProgress(itemId, mediaType, updates) {
  ...
  updated.unshift({ ...entry, ...merged, watchedAt: Date.now() });
  writeContinueWatching(updated);
  return updated;
}
